...
Agent SDKAI 自动化工具编排CI/CD企业级实战

本章带你搞懂 Claude Agent SDK 到底是什么、它和 Claude Code CLI 有什么区别、为什么它能让 AI「自主完成任务」而不只是「回答问题」。我们不只讲概念，而是从安装配置到实战场景，从单个 Agent 到多 Agent 协作，从本地工具到 MCP 集成，把每一个环节都过一遍。学完这章，你就能把 AI 嵌入到自己的自动化流程中，让它真正成为团队的「自动化工程师」。
...
一个能自主修复 bug 的 Python/TypeScript Agent，以及一条完整的 CI/CD 质量守护流水线

📋

前置条件

熟悉 Python 或 TypeScript 基础，学完前几章 Claude Code 使用
...
能写出「自动修 bug」或「代码审查」这样的单任务 Agent

4

会构建流水线

能串联多个 Agent，实现「审查 → 扫描 → 修复 → 测试」的完整流水线

5

掌握高级功能

会用 Hooks 做审计，会用子 Agent 做任务分解，会接入 MCP 服务器

6

记住最佳实践

Prompt 具体化、限制轮数、权限最小化、审计日志、结构化输出
...
-   MCP 服务器：Slack MCP（发通知）
-   权限模式：`bypassPermissions`（只读日志 + 发通知，无风险）
-   定时触发：用 cron 或 GitHub Actions 定时运行脚本
