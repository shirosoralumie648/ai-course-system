...
![PWA 安装与离线流程](week-11-images/pwa-install-offline-flow.png)

*PWA 通过 Manifest、Service Worker 和缓存机制，让网页具备安装和离线访问能力。*
...
![跨平台技术选择地图](week-11-images/cross-platform-map.png)

*技术路线要同时看用户入口、代码复用率和原生体验要求。*
...
下周不会直接进入收官，而是继续把跨平台这条线往前推进：**平台选择与小程序入门。** 你会把本周的 PWA 经验放进更大的平台决策里，判断什么时候该继续做 Web/PWA，什么时候应该进入微信小程序或其他跨端框架。小哲也会带着他的番茄农场 PWA，对照小程序的入口、页面栈、审核和发布流程，重新思考自己的产品到底应该先服务哪个用户场景。
